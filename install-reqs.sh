...
while [ ! -z "$1" ]; do
    case "$1" in
        ...
        --default-qwen)
            ...
            echo "source busevenv/bin/activate;xvfb-run -a python3 $installDir/BROWSER-USE_CONFIGS/agents/default-qwen.py >> $installDir/LOGS/\$(date '+%Y-%m-%d_%H-%M-%S').default-agent.log" > $installDir/agent-launcher.sh
            ;;
